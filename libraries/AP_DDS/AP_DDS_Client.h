...
private:
  ...
  //! @brief Serialize the current gps global origin and publish to the IO
  //! stream(s)
  ...
  //! @brief Serialize the current local velocity and publish to the IO
  //! stream(s)
  ...
  //! @brief Serialize the current local airspeed and publish to the IO
  //! stream(s)
  ...
  //! @brief Serialize the current nav_sat_fix state and publish it to the IO
  //! stream(s)
  void write_battery_state_topic();
  static void update_topic(sensor_msgs_msg_BatteryState &msg,
  ...
  //! @brief Serialize the current nav_sat_fix state and publish to the IO
  //! stream(s)
  void write_nav_sat_fix_topic();
  bool update_topic(sensor_msgs_msg_NavSatFix &msg,
                    const uint8_t instance) WARN_IF_UNUSED;
#endif // AP_DDS_NAVSATFIX_PUB_ENABLED
...
#if AP_DDS_EKF_STATUS_PUB_ENABLED
  ardupilot_msgs_msg_EKFStatus ekf_status_topic;
  // The last ms timestamp AP_DDS wrote an EKF status message
  uint64_t last_ekf_status_time_ms;
  //! @brief Serialize the current EKF status and publish to the IO stream(s)
  void write_ekf_status_topic();
  static void update_topic(ardupilot_msgs_msg_EKFStatus &msg);
#endif // AP_DDS_EKF_STATUS_PUB_ENABLED
...
#if AP_DDS_RANGEFINDER_SUB_ENABLED
  // incoming rangefinder data
  static sensor_msgs_msg_Range rx_rangefinder_topic;
#endif // AP_DDS_RANGEFINDER_SUB_ENABLED
#if AP_DDS_OBSTACLE_DISTANCE_SUB_ENABLED
  ...
  static void on_topic_trampoline(uxrSession *session, uxrObjectId object_id,
  ...
  static void on_request_trampoline(uxrSession *session, uxrObjectId object_id,
                                    uint16_t request_id,
                                    SampleIdentity *sample_id, ucdrBuffer *ub,
                                    uint16_t length, void *args);
  void on_request(uxrSession *session, uxrObjectId object_id,
                  uint16_t request_id, SampleIdentity *sample_id,
                  ucdrBuffer *ub, uint16_t length);

  // delivery control parameters
  uxrDeliveryControl delivery_control{.max_samples = UXR_MAX_SAMPLES_UNLIMITED,
                                      .max_elapsed_time = 0,
  ...
  static size_t udp_transport_write(uxrCustomTransport *transport,
                                    const uint8_t *buf, size_t len,
                                    uint8_t *error);
  static size_t udp_transport_read(uxrCustomTransport *transport, uint8_t *buf,
                                   size_t len, int timeout, uint8_t *error);
