...
static constexpr uint16_t DELAY_STATE_TOPIC_MS = AP_DDS_DELAY_STATE_TOPIC_MS;
...
static constexpr uint16_t DELAY_EKF_STATUS_TOPIC_MS =
    AP_DDS_DELAY_EKF_STATUS_TOPIC_MS;
...
const AP_Param::GroupInfo AP_DDS_Client::var_info[]{
    ...
    // @Description: The maximum number of times the DDS client will attempt to
    // ping the XRCE agent before exiting. Set to 0 to allow unlimited retries.
    // @Range: 0 100
    // @RebootRequired: True
    // @Increment: 1
    // @User: Standard
...
void AP_DDS_Client::update_topic(ardupilot_msgs_msg_State &msg) {
  ...
  msg.armed = armed.is_armed();

  auto *copter = AP::vehicle();
  ...
  // MAV_STATE_EMERGENCY = 6, MAV_STATE_POWEROFF = 7

  // if (copter->any_failsafe_triggered()) {
  // msg.system_status = 5; // MAV_STATE_CRITICAL
  // } else

  if (!copter->is_taking_off() && !copter->is_landing()) {
...
#if AP_DDS_EKF_STATUS_PUB_ENABLED
void AP_DDS_Client::update_topic(ardupilot_msgs_msg_EKFStatus &msg) {
  update_topic(msg.header.stamp);
  STRCPY(msg.header.frame_id, "");
  ...
  msg.velocity_variance = velVar;
  msg.position_variance = posVar;
  msg.height_variance = hgtVar;
  msg.magnetic_variance.x = magVar.x;
  msg.magnetic_variance.y = magVar.y;
  msg.magnetic_variance.z = magVar.z;
  msg.true_airspeed_variance = tasVar;
  msg.healthy = ekf_healthy;
}
#endif // AP_DDS_EKF_STATUS_PUB_ENABLED
...
void AP_DDS_Client::on_topic(uxrSession *uxr_session, uxrObjectId object_id,
  ...
  */
...
#endif // AP_DDS_GLOBAL_POS_CTRL_ENABLED
#if AP_DDS_RANGEFINDER_SUB_ENABLED
  case topics[to_underlying(TopicIndex::RANGEFINDER_SUB)].dr_id.id: {
    const bool success =
        sensor_msgs_msg_Range_deserialize_topic(ub, &rx_rangefinder_topic);
    if (success == false) {
      break;
    }
    AP_DDS_Rangefinder::handle_external_rangefinder(rx_rangefinder_topic);
    break;
  }
#endif // AP_DDS_RANGEFINDER_SUB_ENABLED
#if AP_DDS_OBSTACLE_DISTANCE_SUB_ENABLED
  case topics[to_underlying(TopicIndex::OBSTACLE_DISTANCE_SUB)].dr_id.id: {
    ...
    // Pass to handler for AP_Proximity (pre-arm check) and OA Database
    AP_DDS_ObstacleAvoidance::handle_obstacle_distance(
        rx_obstacle_distance_topic);
    break;
  }
#endif // AP_DDS_OBSTACLE_DISTANCE_SUB_ENABLED
#if AP_DDS_CLOCK_SUB_ENABLED
...
#if AP_DDS_STATE_PUB_ENABLED
void AP_DDS_Client::write_state_topic() {
  WITH_SEMAPHORE(csem);
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
...
#if AP_DDS_EKF_STATUS_PUB_ENABLED
void AP_DDS_Client::write_ekf_status_topic() {
  WITH_SEMAPHORE(csem);
  if (connected) {
    ucdrBuffer ub{};
    const uint32_t topic_size =
        ardupilot_msgs_msg_EKFStatus_size_of_topic(&ekf_status_topic, 0);
    uxr_prepare_output_stream(
        &session, reliable_out,
        topics[to_underlying(TopicIndex::EKF_STATUS_PUB)].dw_id, &ub,
        topic_size);
    const bool success =
        ardupilot_msgs_msg_EKFStatus_serialize_topic(&ub, &ekf_status_topic);
    if (!success) {
      // AP_HAL::panic("FATAL: DDS_Client failed to serialize\n");
    }
  }
...
#endif // AP_DDS_CLOCK_PUB_ENABLED
#if AP_DDS_STATE_PUB_ENABLED
  if (cur_time_ms - last_state_time_ms > DELAY_STATE_TOPIC_MS) {
    update_topic(state_topic);
    last_state_time_ms = cur_time_ms;
    write_state_topic();
  }
#endif // AP_DDS_STATE_PUB_ENABLED
#if AP_DDS_GPS_GLOBAL_ORIGIN_PUB_ENABLED
...
#endif // AP_DDS_GPS_GLOBAL_ORIGIN_PUB_ENABLED
#if AP_DDS_EKF_STATUS_PUB_ENABLED
  if (cur_time_ms - last_ekf_status_time_ms > DELAY_EKF_STATUS_TOPIC_MS) {
    update_topic(ekf_status_topic);
    last_ekf_status_time_ms = cur_time_ms;
    write_ekf_status_topic();
  }
#endif // AP_DDS_EKF_STATUS_PUB_ENABLED
